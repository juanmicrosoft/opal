...
Use this to quantify OPAL's advantages for AI-assisted development.
...
The command finds files with matching base names (e.g., `UserService.opal` and `UserService.cs`) and benchmarks each pair.
...
```bash
opalc benchmark ./src --format markdown --output benchmark.md
```
...
```bash
opalc benchmark ./src --format json --output benchmark.json
```
...
```json
{
  ...
  "results": {
    ...
    "files": [
      {
        "name": "PaymentService",
        "opalPath": "src/PaymentService.opal",
...
- [opalc analyze](/opal/cli/analyze/) - Score files for migration potential
- [opalc convert](/opal/cli/convert/) - Convert files with benchmark option
- [Benchmarking](/opal/benchmarking/) - Detailed methodology documentation
- [Results](/opal/benchmarking/results/) - Published benchmark results
