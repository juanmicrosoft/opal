...
**Result:** 1.09x Calor advantage (estimation mode)
...
§F{f001:FindMax:pub}
  §I{i32[]:arr}
  §O{i32}
  §Q (> (len arr) 0)           // Requires non-empty array

  §V{v001:i32:max} (get arr 0)
  §FOR{i:1:(len arr)}
    §WHEN (> (get arr i) max)
      §SET max (get arr i)
    §/WHEN
...
Both implementations can pass all tests. The benchmark measures which actually does.
...
|:-------|:------|:---|
| Overall Score | 95% | 87% |
| Edge Case Score | 92% | 78% |
| Normal Case Score | 98% | 96% |
| **Advantage Ratio** | **1.09x** | 1.00x |

### By Category

| Category | Calor Score | C# Score | Winner |
|:---------|:------------|:---------|:-------|
| Null Handling | 94% | 82% | Calor |
| Arithmetic Safety | 96% | 88% | Calor |
| String Processing | 92% | 90% | Calor |
| Collection Operations | 96% | 84% | Calor |
| Boundary Conditions | 98% | 92% | Calor |

### Key Finding

Calor's **preconditions and postconditions** provide automatic edge case protection. C# requires explicit guard clauses that LLMs sometimes forget to include.
